...
#from gi.repository import GLib
    ...
    def start_engine(self, path):
        ...
        if path is None:
            path = self.path
            # if using builtin engine return (not UCI)
            if self.engine == "jcchess":
                return

        # path is the path to the UCI engine executable
        if not os.path.isfile(path):
            print("invalid ucipath:", path)
            return False
        ...
        # when jcchess is started on windows with pythonw
        ...
        while True:
            ...
            i += 1
            if i > 40:
                print("error - uciok not returned from engine")
                return False
            time.sleep(0.25)
        ...
        while True:
            ...
            for l in self.op:
                ...
                if l.startswith("bestmove"):
                    ...
                    # update time for last move
                    # print "updating clock from uci.py"
    ...
    def start_ponder(self, pondermove, movelist, cmove):
        ...
        for move in movelist:
            ml = ml + move + " "
        # if ml == "":
        #    print "error empty movelist in ponder in uci.py"
        #    return
    ...
    def get_options(self):
        return self.uci_option

    def set_options(self, options):
        self.uci_option = options
