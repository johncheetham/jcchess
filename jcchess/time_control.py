...
class Time_Control:
    ...
    #
    # get clock settings so they can be saved when quitting jcchess
    #
    ...
    # set the time limit/level when using the builtin jcchess engine
    def set_jcchess_time_limit(self, stm):
        ...
        if self.type == 0:
            ...
            # print "jcchess time limit:",command
            ...
            if idepth > 39:
                if gv.verbose:
                    print(("search depth (", idepth,
                          ") exceeds max for jcchess engine, setting to 39"))
                idepth = 39
            if gv.verbose:
                print("setting depth for jcchess engine to", idepth)
            engine.depth(idepth)
        # fixed nodes search
        elif self.type == 6:
            ...
            if gv.verbose:
                print("setting nodes for jcchess engine to", inodes)
